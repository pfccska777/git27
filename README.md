...
variable "security_group_id" {
  type        = string
  description = "ID группы безопасности из предыдущего задания"
}
...
resource "yandex_compute_instance" "web_servers" {
  ...
  network_interface {
    subnet_id         = "ваш_id_подсети" # Замените на ваш ID
...
locals {
  # Считываем ключ через функцию file
  ssh_public_key = file("~/.ssh/id_rsa.pub")
}

resource "yandex_compute_instance" "db_servers" {
  # Преобразуем list в map для использования for_each
  ...
  # Пункт 4: Создаются ПОСЛЕ web-серверов
